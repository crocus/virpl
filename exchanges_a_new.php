...
switch ( $agency ) {
    ...
    case "значение не задано":
         ...
         break;
    default : 
        $AgeQuery  = $connector . " n.agency_cod = {$agency}";
        break;
}
switch ( $ord ) {
    case NULL:
...
$Exchange = $GenerQuery . $connector . $Type_Exchange . $connector . $formula_pq . $connector . $result_pq . $connector . $uid .  $groupment;
...
if (isset($_GET['action']) && $_GET['action'] == 'submitted') {
 // preparedQuery();
  ...
  echo 'Вероятно Вы допустили ошибку при написании формулы.' . '<br/>';
	...
	echo  '&nbsp;';	
	($formula != "")?  print $_GET['formula'] : print "все предложения.";
