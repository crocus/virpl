...
jQuery.fn.tablePager = function() {
	var opts = jQuery.extend({
		...
		selectors: {
			...
			page: ".pageNum",
			count: ".pageCount"
// modif clem : ajout navigation entre pages (choix page + 1ère et dernière page)
        ,gopage: ".goPage",
        first: ".first",
        last: ".last"
		},
...
// modif clem : ajout navigation entre pages (choix page + 1ère et dernière page)
       , gopage: function() {
				var pagedTable = (arguments.length < 1)?jQuery(this):arguments[0].data;
				var num_page = jQuery(pagedTable[0].tablePager.opts.selectors.gopage).val();
            pagedTable[0].tablePager.showPage(num_page);
				return false;
        },
        first: function() {
				var pagedTable = (arguments.length < 1)?jQuery(this):arguments[0].data;
            pagedTable[0].tablePager.showPage(1);
				return false;
        },
        last: function() {
				var pagedTable = (arguments.length < 1)?jQuery(this):arguments[0].data;
            var rows = pagedTable.find("tbody tr");
            var pagesize = pagedTable[0].tablePager.opts.pagesize;
            var max_page = Math.ceil(rows.length / pagesize);
            pagedTable[0].tablePager.showPage(max_page);
				return false;
        }
		};
...
// modif clem : ajout navigation entre pages (choix page + 1ère et dernière page)
    jQuery(this).find(jQuery(this)[0].tablePager.opts.selectors.gopage).bind("blur", jQuery(this), jQuery(this)[0].tablePager.gopage);
    jQuery(this).find(jQuery(this)[0].tablePager.opts.selectors.first).bind("click", jQuery(this), jQuery(this)[0].tablePager.first);
    jQuery(this).find(jQuery(this)[0].tablePager.opts.selectors.last).bind("click", jQuery(this), jQuery(this)[0].tablePager.last);
	});
};
