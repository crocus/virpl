...
ob_start("ob_gzhandler");
	...
	$GenerQuery = "SELECT f.flats_cod, f.flats_date, f.So, f.Sz, f.Sk, IF (f.flats_cod NOT IN ({$feefor}), f.flats_price, {$marga}) as flats_price,".
	"IF (f.flats_cod NOT IN ({$feefor}), '', {$marga_sum}) as marga, f.kind_calc, f.foto, f.flats_floor, f.flats_floorest, t.type_s, r.room_cod, ci.city_name, a.region_name, s.street_name, w.wc_short, b.balcon_short, m.material_short, n.value_property as agent_name, pp.value_property AS agency_name, f.Source, f.Treated, UNIX_TIMESTAMP(f.last_update) as last_update FROM tbl_flats f
	...
	$count_array["pageNum_Recordset1"]= $pageNum_Recordset1;
	$count_array["maxRows_Recordset1"]=$maxRows_Recordset1;
	$count_array["startRow_Recordset1"]=$startRow_Recordset1;
	$count_array["totalRows_Recordset1"]= $totalRows_Recordset1;
	$count_array["currentPage"]= $currentPage;
	$count_array["totalPages_Recordset1"]= $totalPages_Recordset1;
	$count_array["queryString_Recordset1"]= $queryString_Recordset1;
