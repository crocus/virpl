...
$GenerQuery = "SELECT f.flats_cod, f.flats_date, f.So, f.Sz, f.Sk, f.flats_price, f.foto, f.flats_floor, f.flats_floorest, t.type_s, r.room_cod, a.region_name, s.street_name, w.wc_short, b.balcon_short, m.material_short".
						" FROM tbl_flats f, tbl_type t, tbl_room r, tbl_region a, tbl_street s, tbl_wc w, tbl_balcon b, tbl_material m ".
						"WHERE f.type_cod = t.type_cod  AND f.room_cod = r.room_cod AND f.street_cod = s.street_cod AND s.region_cod = a.region_cod AND f.wc_cod = w.wc_cod AND f.balcon_cod = b.balcon_cod".
						" AND f.material_cod = m.material_cod AND f.sale=0";
$WhereQuery = " And";
$rcon = " And";
$bcon = " And";
$regcon = " And";
...
$SQLQuery = $GenerQuery . $WhereQuery . $TypQuery . $rcon . $romQuery . $bcon . $balQuery . $regcon . $regQuery . $flrcon . $flrQuery . $ordQuery;
...
<form  method="GET" name="form1" action="detail.php"  >
  ...
  <tr> 
    <td height="38" colspan="10" align="center"valign="center" ><font size="2">Объекты с <?php echo ($startRow_Recordset1 + 1) ?> по <?php echo min($startRow_Recordset1 + $maxRows_Recordset1, $totalRows_Recordset1) ?> из <?php echo $totalRows_Recordset1 ?> найденных в базе данных</font></td>
  </tr>
</table>
</form>
</body>
