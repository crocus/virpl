<?php
  ...
  try
  {
    ...
    // Если параметр $_GET['date'] не пуст, запрашиваем глубину
    // за этот день
    if(!empty($_GET['date']))
    {
  ...
  function calendar($date, $min_date)
  {
    ...
    // Выводим таблицу
    echo "<table class=table width=100% border=0 cellpadding=0 cellspacing=0>
            <tr class=header align=center><td align=center colspan=".(count($week) + 1).">Глубина посещения за ".date("Y.m", $date)."</td></tr>";
    for($j = 0; $j < 7; $j++)
    {
      if($j == 5 || $j == 6)
      {
