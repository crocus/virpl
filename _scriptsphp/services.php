<?php
	...
	function upObject($checked_objects) {
		mysql_query("UPDATE tbl_flats SET last_update=Now() WHERE flats_cod in({$checked_objects})") or die("Couldn t execute query.".mysql_error());
	...
	function setParticipantsPhons($prpt,$phons) {
		$prpt_phons = parsePhon($phons);
		$result = mysql_query("SELECT Phone_id FROM tbl_participants_phone WHERE UUID = '{$prpt}'" ) or die("Couldn t execute select query.".mysql_error());
		if (mysql_num_rows($result) > 0 ) {
			mysql_query("DELETE FROM tbl_participants_phone WHERE UUID = '{$prpt}'" ) or die("Couldn t execute delete query.".mysql_error());
		}
		$sql_arr = array();	
		foreach ($prpt_phons as $key => $phon) {			
			$sql_arr[]="(" . $phon. ", '" . $prpt . "')";
		}
		$sql_val = implode(",", $sql_arr);
		$prpt_phons_query = "INSERT INTO tbl_participants_phone (Phone, UUID ) VALUES $sql_val";
		if(mysql_query( $prpt_phons_query )) {
			return 1;
		} else {
	...
	function objectHead($type, $count_r, $city, $region, $street  ) {
		...
		$result = $object_type . $object_place;
		return $result;
	}
?>