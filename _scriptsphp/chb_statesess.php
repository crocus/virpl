	...
	if($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest') {
		...
		switch($action) {
			case "a_update":
				...
				break;
			case "up_checked":
				if (isset( $_SESSION['upChecked']) && !empty($_SESSION['upChecked'])) {
					$sess_array = array_unique(explode(",",(!empty($data['selected']))? $_SESSION['upChecked'] . "," . $data['selected']: $_SESSION['upChecked']));
					$uncheck_array = explode(",", $data['unselected']);
					$a_intersect = array_intersect ($sess_array, $uncheck_array);
					foreach ($a_intersect as $value) {
						$sess_array = str_replace($value, "", $sess_array);
					...
					$sess_array = clear_array_empty($sess_array);
					$current_checked = implode(",", $sess_array);
					$_SESSION['upChecked'] = $current_checked;
				} else {
					$_SESSION['upChecked'] = $data['selected'];
				}
				if (upObject($_SESSION['upChecked'])>0)
					unset($_SESSION ["upChecked"]);
				break;
			case "a_print":
