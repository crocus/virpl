<?php
	...
	function cron_time2string($time) {
		...
		$answer ="еще ";
		if($d > 0){
			return $answer .= cron_formatNumWord($d, _AM_CRON_DAY1, _AM_CRON_DAY2, _AM_CRON_DAY3);
		} elseif ($h > 0){
				return $answer .= cron_formatNumWord($h, _AM_CRON_HOUR1, _AM_CRON_HOUR2, _AM_CRON_HOUR3);
		} elseif ($m > 0){
					return $answer .= cron_formatNumWord($m, _AM_CRON_MIN1, _AM_CRON_MIN2, _AM_CRON_MIN3);
		} else {
			return 'устарело';
		}
	}
